...
interface SelectedExercise {
  ...
  notes: string;
  order_index: number;
  days: number[]; // ← NUEVO: Días seleccionados
}

interface NewRoutineFormWithCategoryProps {
  exercises: Exercise[];
  adminId: string;
}
...
export function NewRoutineFormWithCategory({
  exercises,
  adminId,
}: NewRoutineFormWithCategoryProps) {
  ...
  const addExercise = () => {
    setSelectedExercises([
      ...selectedExercises,
      {
        ...
        days: [1, 2, 3, 4, 5], // Por defecto todos los días
      },
    ]);
  };
  ...
  const toggleDay = (exerciseIndex: number, day: number) => {
    ...
    if (currentDays.includes(day)) {
      // Remover día (pero no permitir array vacío)
      const newDays = currentDays.filter((d) => d !== day);
      if (newDays.length > 0) {
        updated[exerciseIndex].days = newDays;
      }
    } else {
      // Agregar día
      updated[exerciseIndex].days = [...currentDays, day].sort();
    }
